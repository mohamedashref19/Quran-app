...
import {
  ...
  initSearch,
  initBookmarksSearch,
  scheduleFridayKahfNotification,
  scheduleDuhaNotification,
  scheduleAllPrayers,
...
axios.interceptors.response.use(
  ...
  (error) => {
    if (!navigator.onLine || error.message === "Network Error") {
      ...
      const isOfflineHandled = OFFLINE_HANDLED_URLS.some((url) =>
  ...
  try {
      ...
      try {
        if (action.type === "ADD_BOOKMARK") {
          ...
          const found = bookmarks.find(
            (b) =>
              parseInt(b.surah) === parseInt(action.payload.surah) &&
    ...
    if (successCount > 0) {
      ...
      try {
        ...
        console.log(
          `🔄 [SYNC] تم تحديث كاش العلامات بعد الزامنة (${freshBookmarks.length} علامة)`,
        );
      } catch (e) {
        /* تجاهل */
      }
...
const clearExpiredNotifications = async () => {
  ...
  try {
    ...
    const expired = pending.notifications
      .filter(
        (n) =>
          n.schedule &&
          n.schedule.at &&
          new Date(n.schedule.at).getTime() <= now,
      )
      .map((n) => ({ id: n.id }));
    ...
    if (expired.length > 0) {
      ...
      console.log(
        `🧹 [NOTIFICATIONS] تم مسح ${expired.length} إشعار قديم فات وقته لمنع الإزعاج.`,
      );
    }
  } catch (e) {
...
const fetchDynamicSettings = async () => {
  try {
    ...
    console.log(
      "لم يتم تحديث الرابط الديناميكي، سيتم استخدام الرابط الافتراضي",
    );
  }
};
...
window.handleAyahClick = async function (absoluteAyahId, pageNum) {
  ...
  const { surahNum, ayahNum, surahName } = window.getSurahAndAyahFromAbsolute(
    parseInt(absoluteAyahId),
  );
  ...
  const ayahData = pageData.find(
    (a) =>
      (a.ayahNumber || a.numberInSurah) == ayahNum &&
      (a.surah?.number || a.surahNumber) == surahNum,
  );

  const ayahText = ayahData
    ? ayahData.text || ayahData.ayahText
    : "نص الآية غير متوفر";
  ...
  const isBookmarked = userBookmarks.some(
    (b) => parseInt(b.surah) === surahNum && parseInt(b.ayah) === ayahNum,
  );
  ...
  if (sheetEl && window.bootstrap) {
    ...
    const bsSheet =
      bootstrap.Offcanvas.getInstance(sheetEl) ||
      new bootstrap.Offcanvas(sheetEl);
      ...
      function () {
        setTimeout(() => {
          ...
          if (
            backdrop &&
            fullscreenElement &&
            !fullscreenElement.contains(backdrop)
          ) {
...
const tafseerGet = async (surah, ayah) => {
  try {
    ...
    if (cached)
      console.log(
        `⚡ [TAFSEER CACHE HIT] سورة ${surah} - آية ${ayah} - من الكاش`,
      );
    return cached || null;
  } catch (e) {
    return null;
...
window.loadAllUsers = async () => {
  ...
  if (!tbody) return;
  tbody.innerHTML =
    '<tr><td colspan="4" class="text-center py-4"><div class="spinner-border text-success"></div><p>جاري تحميل المستخدمين...</p></td></tr>';
  try {
    ...
    if (!users || users.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="4" class="text-center text-muted">لا يوجد مستخدمين.</td></tr>';
      return;
    }
    users.forEach((user) => {
      const roleBadge =
    ...
    tbody.innerHTML =
      '<tr><td colspan="4" class="text-center text-danger">فشل في تحميل البيانات.</td></tr>';
  }
};
...
window.initLiveStreams = async () => {
  try {
    ...
    const setupVideo = (videoId, urlsArray) => {
      ...
      const errorOverlay = document.getElementById(
        videoId.replace("-video", "-error"),
      );
      ...
      video.addEventListener("play", () => {
        ...
        const allLiveVideos = document.querySelectorAll(
          "#live-stream-pane video",
        );
          ...
          hls = new Hls({
            ...
            xhrSetup: function (xhr, url) {
              ...
              if (
                ...
                url.includes("_capacitor_http_interceptor_")
              ) {
                try {
                  // استخراج الرابط الأصلي من داخل رابط Capacitor المعقد
                  const originalUrlParam = new URL(url).searchParams.get("u");
                  let finalUrl = originalUrlParam
                    ? decodeURIComponent(originalUrlParam)
                    : url;

                  // لو الرابط لسة بايز وبيشاور على localhost (زي chunklist)
                  if (finalUrl.includes("localhost")) {
                    const baseUrl = currentStreamUrl.substring(
                      0,
                      currentStreamUrl.lastIndexOf("/") + 1,
                    );
                    const fileName = finalUrl.substring(
...
window.submitToLeaderboard = async (score, total, dateStr) => {
  ...
  if (!nameInput) {
    statusDiv.innerHTML =
      '<span class="text-danger">يرجى كتابة اسمك أولاً!</span>';
    return;
  }
  ...
  try {
    ...
    statusDiv.innerHTML =
      '<span class="text-success fw-bold"><i class="fas fa-check-circle me-1"></i> تم تسجيل نتيجتك بنجاح!</span>';
    btn.innerHTML = "تم ✓";
    ...
    btn.innerHTML = "شارك";
    statusDiv.innerHTML =
      '<span class="text-danger">حدث خطأ في الاتصال، حاول مرة أخرى.</span>';
  }
};
document.addEventListener("DOMContentLoaded", () => {
  ...
  setTimeout(() => {
    window.downloadEntireTafseerOffline();
  }, 10000);
  ...
  if (typeof Capacitor !== "undefined" && Capacitor.isNativePlatform()) {
    ...
    LocalNotifications.addListener(
      ...
      async (action) => {
        ...
        if (
          notification.id === 999 ||
          notification.actionTypeId === "OPEN_KHATMAH"
        ) {
...
window.stopAllMedia = () => {
  ...
  if (window.currentPlayingAudio) {
    ...
    if (window.currentPlayingBtn) {
      window.currentPlayingBtn.innerHTML =
  ...
  if (
    typeof aiMediaRecorder !== "undefined" &&
    aiMediaRecorder &&
    aiMediaRecorder.state !== "inactive"
  ) {
  ...
  if (
    typeof chunkRecorder !== "undefined" &&
    chunkRecorder &&
    chunkRecorder.state !== "inactive"
  ) {
    try {
      chunkRecorder.stop();
    } catch (e) {}
...
const resetUIButtons = () => {
  ...
  if (startLive && stopLive) {
    ...
    if (liveSt) {
      liveSt.innerText = "جاهز...";
      liveSt.className = "text-muted small mt-1";
    }
  }
...
window.checkAuth = async () => {
  ...
  if (!savedToken) {
    ...
    document
      .querySelectorAll(".user-link, .admin-link")
  ...
  try {
    ...
    if (err.response?.status === 401 && savedToken) {
      console.warn(
        "⚠️ [AUTH] 401 رغم وجود توكن - إزالة التوكن وعرض شاشة الدخول",
      );
      if (Capacitor.isNativePlatform()) {
      ...
      <div class="col-6 col-md-4 col-lg-3">
        ...
        i + 1
          ...
          <div class="card-body p-2">
            <span class="badge bg-light text-dark mb-1 border rounded-circle">${
              i + 1
            }</span>
  ...
  juzData.forEach((juz) => {
    ...
    html += `
      <div class="juz-header">
        <div><h5><i class="fas fa-book-open me-2"></i>الجزء ${
          juz.juz
        }</h5><small style="opacity:0.85">${juz.name}</small></div>
        <div class="text-end"><div class="juz-badge mb-1">ص ${
          juz.page
        }</div><small style="opacity:0.8; font-size:0.75rem">${
      juz.surahs
    }</small></div>
      </div>
          ...
          <div class="card bg-transparent border-success h-100 hover-shadow" style="cursor:pointer; border-width:2px !important;"
            ...
            <div class="card-body text-center py-3">
              ...
              <div class="justify-content-center d-flex flex-wrap gap-2">
                ...
                <button class="btn btn-sm btn-success text-white rounded-pill px-2 py-1" onclick="event.stopPropagation(); window.showSection('quran'); window.loadQuranPage(${
                  hizb1Page + 2
                });">ص ${hizb1Page + 2} الربع 2</button>
                <button class="btn btn-sm btn-success text-white rounded-pill px-2 py-1" onclick="event.stopPropagation(); window.showSection('quran'); window.loadQuranPage(${
                  hizb1Page + 5
                });">ص ${hizb1Page + 5} الربع 3</button>
                <button class="btn btn-sm btn-success text-white rounded-pill px-2 py-1" onclick="event.stopPropagation(); window.showSection('quran'); window.loadQuranPage(${
                  hizb1Page + 7
                });">ص ${hizb1Page + 7} الربع 4</button>
              </div>
            </div>
          </div>
        </div>
          ...
          <div class="card bg-transparent border-success h-100 hover-shadow" style="cursor:pointer; border-width:2px !important;"
            ...
            <div class="card-body text-center py-3">
              ...
              <div class="justify-content-center d-flex flex-wrap gap-2">
                ...
                <button class="btn btn-sm btn-success text-white rounded-pill px-2 py-1" onclick="event.stopPropagation(); window.showSection('quran'); window.loadQuranPage(${
                  hizb2Page + 2
                });">ص ${hizb2Page + 2} الربع 2</button>
                <button class="btn btn-sm btn-success text-white rounded-pill px-2 py-1" onclick="event.stopPropagation(); window.showSection('quran'); window.loadQuranPage(${
                  hizb2Page + 5
                });">ص ${hizb2Page + 5} الربع 3</button>
                <button class="btn btn-sm btn-success text-white rounded-pill px-2 py-1" onclick="event.stopPropagation(); window.showSection('quran'); window.loadQuranPage(${
                  hizb2Page + 7
                });">ص ${hizb2Page + 7} الربع 4</button>
              </div>
            </div>
          </div>
        </div>
  ...
  try {
    ...
    if (cached) {
      ...
      Swal.fire({
        icon: "error",
        title: "لا يوجد اتصال",
        text: "التفسير غير متاح بدون إنترنت حتى الآن",
      });
    }
  }
};
...
window.showSection = async (sectionName) => {
  ...
  if (sectionName !== "quran" && typeof _stopAutoScroll === "function")
    _stopAutoScroll();
  document
    ...
    axios
      ...
      .then(async (res) => {
        ...
        if (u) {
          ...
          await localforage.setItem("cached_user_profile", {
            name: u.name,
            email: u.email,
          });
        }
      ...
      .catch(async (err) => {
        if (
          ...
          if (cachedUser) {
            const n = document.getElementById("profile-name");
            if (n) n.value = cachedUser.name;
            const e = document.getElementById("profile-email");
            if (e) e.value = cachedUser.email;
            return;
          }
        }
  ...
  if (sectionName !== "qibla") {
    window.removeEventListener(
      ...
      window._qiblaOrientationHandler,
    );
    window.removeEventListener(
      "deviceorientation",
      window._qiblaOrientationHandler,
...
window.openQuranAtCurrentKhatmah = async () => {
  try {
    ...
    const targetPage =
      savedPage >= surahFirstPage && savedPage <= 604
        ? savedPage
        : surahFirstPage;
...
window.loadLiveAyahs = async () => {
  ...
  const startAyah =
    parseInt(document.getElementById("live-start-ayah").value) || 1;
  const endAyah =
    parseInt(document.getElementById("live-end-ayah").value) || 999;
  ...
  container.innerHTML =
    '<div class="text-center py-4"><div class="spinner-border text-success"></div><p>جاري جلب الآيات...</p></div>';
  ...
  try {
    ...
    const filteredAyahs = data.data.ayahs.filter(
    ...
    if (!filteredAyahs.length) {
      container.innerHTML =
        '<p class="text-muted">لا توجد آيات في هذا النطاق.</p>';
      return;
    }
  ...
  if (typeof isLiveTracking !== "undefined" && isLiveTracking) {
    if (typeof Swal !== "undefined") {
      Swal.fire({
        ...
        position: "bottom",
        icon: "warning",
        title: "أوقف التسميع أولاً قبل تشغيل الشيخ",
        showConfirmButton: false,
        timer: 3000,
      });
    }
...
function _attachWordTracking(audioEl, ayahIndex, skipBasmala) {
  ...
  const handler = () => {
    ...
    const currentAyahEl =
      document.querySelectorAll(".live-ayah-item")[ayahIndex];
...
function _playSheikhAyah(index) {
  ...
  if (!ayahs || index >= ayahs.length) {
    ...
    if (statusEl)
      statusEl.innerHTML =
        '<span class="text-success fw-bold">✅ انتهت التلاوة</span>';
    return;
  }
  ...
  const playMainAyah = () => {
    ...
    if (statusEl)
      statusEl.innerHTML = `<span class="text-success small"><i class="fas fa-volume-up fa-pulse me-1"></i> الآية ${ayah.num} من ${ayahs.length}</span>`;
...
document.addEventListener("visibilitychange", () => {
  if (document.hidden) {
    if (
      typeof stopLiveTracking === "function" &&
      typeof isLiveTracking !== "undefined" &&
      isLiveTracking
    ) {
      stopLiveTracking();
      resetUIButtons();
...
window.addEventListener("popstate", () => {
  ...
  if (
    typeof stopSheikhFollowAlong === "function" &&
    typeof _sheikPlaybackActive !== "undefined" &&
    _sheikPlaybackActive
  ) {
...
document.addEventListener("click", async (e) => {
  ...
  if (e.target.closest(".action-btn-tafseer")) {
    ...
    if (data) {
      if (typeof window.showTafseer === "function")
        window.showTafseer(data.surah, data.ayah);
      else if (typeof window.showTafseerModal === "function")
        window.showTafseerModal(data.surah, data.ayah);
      else if (typeof showTafseerModal !== "undefined")
        showTafseerModal(data.surah, data.ayah);
    }
    return;
  }
  ...
  if (e.target.closest(".action-btn-nuzul")) {
    ...
    const data = window.selectedVerseData;
    if (data) {
      if (typeof window.openAyahInsights === "function")
        window.openAyahInsights(data.surah, data.ayah);
      else if (typeof window.showNuzulModal === "function")
        window.showNuzulModal(data);
  ...
  if (e.target.closest(".action-btn-bookmark")) {
    ...
    if (typeof isUserLoggedIn === "function" && !(await isUserLoggedIn())) {
      requireLogin("استخدام العلامات المرجعية");
      return;
    }
    ...
    if (data) {
      ...
      if (typeof toggleBookmark === "function")
        await toggleBookmark(data.surah, data.ayah, dummyIcon);
  ...
  if (e.target.closest(".action-btn-khatmah")) {
    if (typeof hideVerseSheet === "function") await hideVerseSheet();
    if (typeof isUserLoggedIn === "function" && !(await isUserLoggedIn())) {
      requireLogin("تتبع الختمة");
      return;
    }

    const data = window.selectedVerseData;
    if (data) {
      if (typeof updateKhatmahProgress === "function")
        await updateKhatmahProgress(data.surah, data.ayah);
      if (window.loadQuranPage) window.loadQuranPage(window.currentPage);
    }
    return;
  }
  ...
  if (e.target.closest(".action-btn-share")) {
    ...
    if (data) {
      ...
      if (typeof window.shareAyah === "function")
        window.shareAyah(data.text, data.surahName, data.ayah);
      else if (typeof shareAyah === "function")
        shareAyah(data.text, data.surahName, data.ayah);
    }
    return;
  }
...
function _calcStats() {
  ...
  const best = Object.values(stats).length
  ...
  let levelName = "بذرة الإيمان";
  let levelIcon = "🌱";
  let levelColor = "text-secondary";
  if (total >= 604) {
    levelName = "تاج الوقار (خاتم)";
    levelIcon = "👑";
    levelColor = "text-warning";
  } else if (total >= 300) {
    levelName = "صاحب القرآن";
    levelIcon = "🌟";
    levelColor = "text-primary";
  } else if (total >= 100) {
    levelName = "القارئ المواظب";
    levelIcon = "📘";
    levelColor = "text-success";
  } else if (total >= 30) {
    levelName = "محب التلاوة";
    levelIcon = "📖";
...
function _renderStats() {
  ...
  s.last7.forEach((val, i) => {
    ...
    bar.style.cssText = `
      ...
      background:${
        isToday
          ? "linear-gradient(to top,#1e5f31,#4caf50)"
          : val > 0
          ? "#a5d6a7"
          : "#e8f5e9"
      };
    ...
    if (val > 0) {
      ...
      num.style.cssText =
        "position:absolute;top:-18px;left:50%;transform:translateX(-50%);font-size:0.65rem;color:#555;white-space:nowrap;";
      num.textContent = val;
      bar.appendChild(num);
    }
    ...
    lbl.style.cssText = `flex:1;text-align:center;font-size:0.65rem;color:${
      isToday ? "#198754" : "#999"
...
// ══════════════════════════════════════════════════════════════════════════════
const ZOOM_STEPS = [
  0.55, 0.65, 0.75, 0.85, 1.0, 1.1, 1.2, 1.35, 1.5, 1.7, 2.0, 2.3, 2.6, 3.0,
];
const ZOOM_LABELS = [
  "55%",
  "65%",
...
window.quranZoom = function (direction) {
  _zoomIndex = Math.max(
    0,
    Math.min(ZOOM_STEPS.length - 1, _zoomIndex + direction),
  );
  _applyZoom();
...
function _isOnQuranPage() {
  ...
  const isTextFullscreen =
    document.body.classList.contains("fullscreen-reading") &&
...
function _updateAutoScrollUI(isActive) {
  ...
  if (isActive) {
    if (btnN) {
      btnN.classList.replace("text-secondary", "text-success");
      btnN.innerHTML = '<i class="fas fa-pause me-1"></i> إيقاف';
    }
    if (ctlN) {
      ctlN.classList.replace("d-none", "d-flex");
    }

    if (btnF) {
      btnF.innerHTML = '<i class="fas fa-pause me-1"></i> إيقاف';
      btnF.classList.add("text-danger");
    }
    if (ctlF) {
      ctlF.style.setProperty("display", "flex", "important");
    }
    if (fabF) {
      fabF.style.setProperty("display", "flex", "important");
    }
    ...
    if (btnN) {
      btnN.classList.replace("text-success", "text-secondary");
      btnN.innerHTML = '<i class="fas fa-scroll me-1"></i> تمرير';
    }
    if (ctlN) {
      ctlN.classList.replace("d-flex", "d-none");
    }

    if (btnF) {
      btnF.innerHTML = '<i class="fas fa-scroll me-1"></i> تمرير';
      btnF.classList.remove("text-danger");
    }
    if (ctlF) {
      ctlF.style.setProperty("display", "none", "important");
    }
    if (fabF) {
      fabF.style.setProperty("display", "none", "important");
    }
...
function _runScrollLoop() {
  ...
  const contentContainer =
    isFullScreen && activePage
      ? activePage
      : document.getElementById("quran-book");
  ...
  if (!spacer) {
    ...
    spacer.innerHTML =
      '<i class="fas fa-chevron-down fa-2x fa-fade text-success"></i>';
...
(function () {
  ...
  async function _loadCoordinatesCSV() {
    ...
    try {
      ...
      console.log(
        "✅ تم تحميل إحداثيات المصحف بنجاح!",
      ...
      if (res.ok) {
        ...
        setTimeout(() => {
          ...
          if (nextPage >= 1 && !_pageCache[nextPage]) {
            fetch(`/assets/quran_pages/${nextPage}.json`)
              .then((r) => r.json())
              .then((d) => {
                _pageCache[nextPage] = d?.data?.ayahs || d?.ayahs || d || [];
              })
              .catch(() => {});
          }
          if (prevPage <= 604 && !_pageCache[prevPage]) {
            fetch(`/assets/quran_pages/${prevPage}.json`)
              .then((r) => r.json())
              .then((d) => {
                _pageCache[prevPage] = d?.data?.ayahs || d?.ayahs || d || [];
              })
              .catch(() => {});
          }
        }, 300);
  ...
  async function _loadPageContent(pageEl, pageNum) {
    ...
    const drawDebugDots = () => {
      ...
      pageCoords.forEach((coord) => {
        ...
        lbl.style.cssText =
          "position:absolute; top:-18px; left:-8px; font-size:11px; color:yellow; font-weight:bold; background:rgba(0,0,0,0.7); padding:1px 4px; border-radius:3px; pointer-events:none;";
        dot.appendChild(lbl);
  ...
  function _preloadImg(pageNum) {
    if (pageNum < 1 || pageNum > TOTAL_PAGES || _imgPreloadCache[pageNum])
      return;
  ...
  function _setupTrack(currentPage) {
    ...
    if (_swipeTrack.children.length !== 3) {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        slot.style.cssText =
          "flex:0 0 100vw;width:100vw;height:100%;overflow:hidden;position:relative;";
  ...
  function _swipeToPage(newPage) {
    ...
    requestAnimationFrame(() => {
      ...
      const pageLabel =
        document.getElementById("mushaf-page-label") ||
      ...
      (e) => {
        ...
        if (window._imageMushafActive && _swipeTrack) {
          ...
          _rafId = requestAnimationFrame(() => {
            ...
            if (
              (window.currentPage === 1 && dx > 0) ||
              (window.currentPage === TOTAL_PAGES && dx < 0)
            ) {
              resistance = 0.3;
            }
            _swipeTrack.style.transform = `translateX(${
              _startTransform + dx * resistance
            }px)`;
          });
        } else {
    ...
    document.addEventListener(
      ...
      (e) => {
        ...
        if (window._imageMushafActive && _swipeTrack) {
          ...
          if (dx < 0) {
            // سحب لليسار (يقلب للصفحة التالية في المصحف)
            if (window.currentPage > 1) _swipeToPage(window.currentPage - 1);
            else _centerTrack();
          } else {
            // سحب لليمين (يقلب للصفحة السابقة)
            if (window.currentPage < TOTAL_PAGES)
              _swipeToPage(window.currentPage + 1);
            else _centerTrack();
          }
        }
  ...
  if (_origLoad) {
    window.loadQuranPage = async function (
      pageNum,
      surahNum,
      ayahNum,
      ...rest
      ...
      if (
        isImageMushafMode &&
        !document.getElementById("quran-pages-track-wrapper")
      ) {
        _initSwipeUI();
      }

      const result = await _origLoad.call(
        this,
        targetPage,
        surahNum,
        ayahNum,
      ...
      if (
        isImageMushafMode &&
        document.body.classList.contains("swipe-nav-active") &&
        typeof _setupTrack === "function"
      ) {
        ...
        if (surahNum) {
          setTimeout(() => {
            const activePage = document.querySelector(
              ".quran-swipe-page.active-page",
            );
            if (activePage) {
              let targetEl = null;
              if (ayahNum && parseInt(ayahNum) > 1) {
                targetEl = activePage.querySelector(
                  `#ayah-${surahNum}-${ayahNum}`,
                );
              } else {
                targetEl = activePage.querySelector(
                  `#surah-header-${surahNum}`,
                );
              }
              if (targetEl) {
        ...
        setTimeout(() => {
          ...
          if (targetEl) {
            ...
            const navH =
              (document.getElementById("main-navbar")?.offsetHeight || 60) + 10;
  ...
  document.addEventListener("DOMContentLoaded", () => {
      ...
      if (qs && !qs.classList.contains("d-none")) {
        if (
          ...
          setTimeout(() => {
            if (
              window.currentPage &&
              !document.querySelector(".quran-swipe-page")
            ) {
    ...
    const qs = document.getElementById("quran-section");
    if (qs)
      observer.observe(qs, { attributes: true, attributeFilter: ["class"] });
  });
  ...
  document.addEventListener("fullscreenchange", () => {
    if (document.fullscreenElement) {
      ...
      const wasVideo =
        window._lastFullscreenElement &&
        window._lastFullscreenElement.tagName === "VIDEO";
...
if (quranContainer) {
  quranContainer.addEventListener(
    "click",
    function (e) {
      ...
      if (
        isAyah.hasAttribute("data-bs-toggle") ||
        verseWrapper.hasAttribute("data-bs-toggle")
      ) {
        const target =
          isAyah.getAttribute("data-bs-target") ||
          verseWrapper.getAttribute("data-bs-target");
      ...
      if (_clickCount === 1) {
        _clickTimer = setTimeout(() => {
          ...
          const isBookmarked =
            verseWrapper.getAttribute("data-bookmarked") === "true";
          ...
          const bookmarkBtnText = document.getElementById(
            "sheet-bookmark-text",
          );
          const bookmarkIcon = document.getElementById("sheet-bookmark-icon");
          if (bookmarkBtnText && bookmarkIcon) {
          ...
          const targetId =
            verseWrapper.getAttribute("data-saved-target") ||
            "#verseActionSheet";
          const sheetEl = document.querySelector(targetId);
          if (sheetEl && window.bootstrap) {
            const bsSheet =
              window.bootstrap.Offcanvas.getOrCreateInstance(sheetEl);
            bsSheet.show();
          }
        }, 300);
...
window.addEventListener("popstate", (event) => {
  ...
  if (event.state?.section) {
    document
      ...
      .forEach((el) => el.classList.add("d-none"));
    document
      .getElementById(`${event.state.section}-section`)
...
const initNativeFeatures = async () => {
  ...
  try {
    ...
    if (savedVersion !== currentVersion) {
      console.log(
        `🔄 [UPDATE] تم تحديث التطبيق إلى ${currentVersion} — جاري مسح الكاش والـ channels القديمة`,
      );
      ...
      try {
        await LocalNotifications.deleteChannel({ id: "khatmah-channel" });
      } catch (e) {}
      try {
        await LocalNotifications.deleteChannel({ id: "azan-channel" });
      } catch (e) {}
      try {
        await LocalNotifications.deleteChannel({ id: "quiz-channel-silent" });
      } catch (e) {}
    }
    ...
    await App.addListener("appUrlOpen", ({ url }) => {
      try {
        ...
        if (
          urlObj.pathname === "/resetPassword" ||
          urlObj.pathname === "/reset-password.html"
        ) {
    ...
    if (notifs.display === "granted") {
      ...
      if (savedPrayers && savedPrayers.timings) {
        await scheduleAllPrayers({
          timings: savedPrayers.timings,
          rawTimestamps: savedPrayers.rawTimestamps,
        });
      } else {
        console.log(
          "⚠️ [PRAYERS] لا توجد مواقيت محفوظة، سيتم الجدولة عند فتح صفحة الصلاة",
        );
      }
...
const checkForUpdates = async () => {
  ...
  try {
    ...
    const modalHtml = `
      <div class="modal fade" id="modal-update" tabindex="-1" data-bs-backdrop="${
        isForce ? "static" : "true"
      }" dir="rtl">
        <div class="modal-dialog modal-dialog-centered">
          <div class="modal-content border-0 shadow-lg" style="border-radius:20px;overflow:hidden;background:#ffffff !important;font-family:'Amiri',sans-serif;">
            <div style="background:linear-gradient(135deg,#1e5f31,#198754);padding:28px 24px 20px;text-align:center;color:white;">
              ...
              <p class="mb-0" style="font-size:0.9rem;color:#e8f5e9 !important;">الإصدار ${
                data.version
              } جاهز للتحميل</p>
            </div>
            <div class="modal-body p-4" style="background:#ffffff !important;">
              <div style="border-radius:12px;background:#f1f8e9 !important;border:1px solid #c8e6c9;padding:14px 16px;margin-bottom:16px;">
                ...
                <div style="font-size:0.9rem;color:#333 !important;line-height:1.8;">${
                  data.releaseNotes
                }</div>
              </div>
              ${
                isForce
                  ? `<div style="border-radius:12px;background:#fff3e0 !important;border:1px solid #ffcc80;padding:12px 16px;text-align:center;"><span style="font-size:0.85rem;color:#e65100 !important;font-weight:600;">⚠️ هذا التحديث إجباري ويجب التحديث للمتابعة</span></div>`
                  : ""
              }
            </div>
            <div class="modal-footer border-0 pb-4 pt-0 d-flex flex-column gap-2 px-4" style="background:#ffffff !important;">
              ...
              ${
                !isForce
                  ? `<button class="btn w-100 py-2 fw-semibold" data-bs-dismiss="modal" style="border-radius:12px;background:#eeeeee !important;color:#555 !important;border:none;">لاحقاً</button>`
                  : ""
              }
            </div>
          </div>
...
window.shareDailyContent = async function () {
  if (typeof html2canvas === "undefined") {
    Swal.fire({
      icon: "error",
      title: "عفواً",
      text: "مكتبة الصور غير محملة.",
    });
    return;
  }

  const isAyah = !document
    .getElementById("daily-ayah-content")
  ...
  const defaultLink =
    "https://play.google.com/store/apps/details?id=com.mohamedashraf.aqra";
  const dynamicShareLink =
    localStorage.getItem("dynamic_share_url") || defaultLink;
...
window.toggleRadio = function () {
  ...
  const hiddenInput = document.getElementById("radio-station-url"); // 🌟 التعديل هنا
  const status = document.getElementById("radio-status");
  const playIcon = document.getElementById("radio-play-icon");
  const radioIcon = document.getElementById("radio-icon");

  const tryBackup = () => {
    const backupUrl = hiddenInput.getAttribute("data-backup"); // 🌟 التعديل هنا

    if (backupUrl && audio.src !== backupUrl) {
      console.warn("⚠️ الرابط الأساسي تعطل، جاري محاولة الرابط البديل...");
      status.innerHTML =
        '<i class="fas fa-sync fa-spin me-1"></i> محاولة رابط احتياطي...';
      audio.src = backupUrl;
      audio.load();
      audio
        .play()
        ...
        .catch((e) => {
          status.innerHTML =
            '<i class="fas fa-times text-danger me-1"></i> جميع الروابط معطلة';
        });
    } else {
      status.innerHTML =
        '<i class="fas fa-times text-danger me-1"></i> المحطة غير متاحة حالياً';
    }
  };
  ...
  function updateUIOnSuccess() {
    ...
    status.innerHTML =
      '<i class="fas fa-circle text-danger me-1 blink-animation"></i> بث مباشر';
  ...
  status.innerHTML =
    '<i class="fas fa-spinner fa-spin me-1"></i> جاري الاتصال...';
  audio.src = hiddenInput.value; // 🌟 التعديل هنا
  audio.load();
...
// الاستماع لضغطات المستخدم داخل النافذة المنبثقة
document.addEventListener("click", function (e) {
  const stationBtn = e.target.closest(".station-item");
  if (!stationBtn) return;

  // استخراج البيانات من الزرار اللي تم الضغط عليه
  const url = stationBtn.getAttribute("data-url");
  const backup = stationBtn.getAttribute("data-backup");
  const name = stationBtn.getAttribute("data-name");
  ...
  // تحديث الواجهة
  stationNameSpan.innerText = name;
  hiddenInput.value = url;
  hiddenInput.setAttribute("data-backup", backup);

  // تلوين المحطة المختارة في الـ Modal وإلغاء تلوين الباقي
  document.querySelectorAll(".station-item").forEach((btn) => {
    btn.classList.remove("bg-success", "text-white");
    btn.style.color = "#1e5f31";
    btn.querySelector("i").classList.replace("text-white", "text-success");
  });
  stationBtn.classList.add("bg-success", "text-white");
  stationBtn.style.color = "white";
  stationBtn.querySelector("i").classList.replace("text-success", "text-white");

  // إغلاق النافذة المنبثقة تلقائياً
  const modalEl = document.getElementById("radioStationsModal");
  ...
  if (!audio.paused) {
    status.innerHTML =
      '<i class="fas fa-spinner fa-spin me-1"></i> تبديل المحطة...';
    audio.pause();
    audio.src = "";
    window.toggleRadio();
  } else {
...
window.loadNamesOfAllah = function () {
  ...
  window.renderAllahNames(allahNamesData);
  container.dataset.loaded = "true";
  ...
  myCarouselEl.addEventListener("slide.bs.carousel", function (e) {
    const total = document.querySelectorAll(
      "#names-carousel-inner .carousel-item",
    ).length;
...
window.renderAllahNames = function (data) {
  ...
  if (!data || data.length === 0) {
    container.innerHTML =
      '<div class="text-center text-danger p-4 fw-bold mt-4"><i class="fas fa-search-minus fa-3x mb-3"></i><br>لم يتم العثور على هذا الاسم</div>';
...
window.filterAllahNames = function () {
  ...
  const filtered = allahNamesData.filter((item) => {
    return (
      cleanText(item.n).includes(cleanQuery) ||
      cleanText(item.m).includes(cleanQuery)
    );
  });
...
window.initIslamicCountdown = function () {
  ...
  if (nextEvent) {
    ...
    const note =
      diffDays > 2
        ? ' <span style="font-size: 0.65rem; opacity: 0.7; font-weight: normal; margin-right: 2px;">(فلكياً)</span>'
        : "";
    ...
    if (diffDays === 0 || diffDays === 1) {
      ...
      if (!localStorage.getItem(celebrationKey)) {
        setTimeout(() => {
          ...
          const cleanName = nextEvent.name
            .replace(/🌙|🎉|🕋|🐑|📅/g, "")
            .trim();
          Swal.fire({
...
window.openAzkarCategory = function (type) {
  ...
  if (AZKAR_AUDIO_LINKS[type]) {
    ...
    audioPlayer.onloadedmetadata = () => {
      document.getElementById("azkar-duration").innerText = formatAudioTime(
    ...
    audioPlayer.ontimeupdate = () => {
      ...
      document.getElementById("azkar-current-time").innerText =
...
window.openAyahInsights = function (surahNum, ayahNum) {
  ...
  const surahData = window.quranInsights.surahs[surahNum] || {
    ...
    theme:
      "هذه السورة تتضمن توجيهات إلهية ومقاصد عظيمة تدعو للتفكر في آيات الله والعمل بمنهجه.",
    fadl: "",
  };
...
window.renderAsbabAlNuzul = function () {
  ...
  if (Object.keys(verses).length === 0) {
    for (const key in window.quranInsights) {
      if (key !== "surahs" && key.includes("_"))
        verses[key] = window.quranInsights[key];
    }
  }
  ...
  for (let i = 1; i <= 114; i++) {
    ...
    if (surahData || storiesCount > 0) {
      const surahName =
        surahData?.name ||
        (window.surahNames ? window.surahNames[i - 1] : `سورة ${i}`);
      const badgeHtml =
        storiesCount > 0
...
window.searchStories = function () {
  const input = document
    .getElementById("stories-search-input")
    .value.toLowerCase();
...
window.searchInnerSurah = function () {
  const input = document
    .getElementById("inner-surah-search")
    .value.toLowerCase();
  const items = document.querySelectorAll(
    "#surah-details-content .inner-story-item",
  );
...
window.openSurahDetails = function (surahNum) {
  ...
  const surahName =
    surahData?.name ||
    (window.surahNames ? window.surahNames[surahNum - 1] : `سورة ${surahNum}`);

  let verses = window.quranInsights?.verses || {};
  if (Object.keys(verses).length === 0) {
    for (const k in window.quranInsights) {
      if (k !== "surahs" && k.includes("_"))
        verses[k] = window.quranInsights[k];
    }
  }
  ...
  if (surahData) {
    ...
    if (surahData.theme)
      contentHtml += `<p class="text-dark small lh-lg mb-2"><strong class="badge bg-success me-1">المقصد العام:</strong> <br> <span class="d-block mt-1">${surahData.theme}</span></p>`;
    if (surahData.fadl)
      contentHtml += `<p class="text-dark small lh-lg mb-2"><strong class="badge bg-warning text-dark me-1">فضلها:</strong> <br> <span class="d-block mt-1">${surahData.fadl}</span></p>`;
    if (surahData.action)
      contentHtml += `<p class="text-dark small lh-lg mb-0"><strong class="badge bg-info text-dark me-1">تأمل وعمل:</strong> <br> <span class="d-block mt-1">${surahData.action}</span></p>`;
    contentHtml += `</div>`;
  }
...
window.openPersonDetails = function (type, index) {
  let data =
    type === "prophet"
      ? window.quranInsights?.prophets?.[index]
      : window.quranInsights?.sahaba?.[index];
  if (!data) return;
  ...
  let categoryBadgeHtml = categoryTag
    ? `<span class="badge bg-secondary mb-3 shadow-sm"><i class="fas fa-bookmark me-1"></i> ${categoryTag}</span><br>`
    : "";
  ...
  let contentHtml = `
    ...
    type === "prophet" ? "text-dark" : ""
  } fs-6 px-3 py-2 rounded-pill shadow-sm mb-4">${data.title}</span>
        
        <div class="text-start">
            <h6 class="fw-bold text-dark border-bottom pb-2 mb-3"><i class="fas fa-book-open me-2 text-muted"></i> القصة والسيرة</h6>
            <p class="text-muted lh-lg mb-4" style="font-size: 1.05rem; white-space: pre-wrap;">${
              data.story
            }</p>
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (initialPath === "/" || initialPath === "/index.html") {
    ...
    initialPath.includes("manage-users")
  ) {
  ...
  if (editProfileBtn) {
    editProfileBtn.addEventListener("click", () => {
      ...
      if (isCurrentlyDisabled) {
        ...
        editProfileBtn.classList.replace(
          "btn-outline-success",
          "btn-outline-danger",
        );
  ...
  handleForm("resetPasswordFormPage", () => {
    ...
    if (newPass.length < 8)
      return showAlert("error", "كلمة المرور يجب أن تكون 8 أحرف على الأقل");
    if (newPass !== confirmPass)
      return showAlert("error", "كلمتا المرور غير متطابقتين");
  ...
  if (uploadBtn) {
    uploadBtn.addEventListener("click", (e) => {
      ...
      if (!audioFile.files[0])
        return showAlert("error", "يرجى اختيار ملف أولاً");
      ...
      checkRecitation(
        ...
        startAyah,
        endAyah,
        URL.createObjectURL(audioFile.files[0]),
      );
    });
  }
  ...
  // Khatmah Buttons
  document
    .getElementById("specialBookmarkBtn")
    ?.addEventListener("click", async () => {
  ...
  document
    .getElementById("deleteKhatmahBtn")
    ?.addEventListener("click", async () => {
      const result = await Swal.fire({
        title: "هل أنت متأكد؟",
        text: "سيتم إلغاء الختمة الحالية نهائياً",
        icon: "warning",
        showCancelButton: true,
        confirmButtonColor: "#d33",
        cancelButtonColor: "#198754",
        confirmButtonText: "نعم، إلغاء الختمة",
        cancelButtonText: "تراجع",
      });
  ...
  document
    .getElementById("volume-slider-live")
  ...
  for (let i = 0; i < localStorage.length; i++) {
    ...
    if (value && value.includes("{")) {
      try {
        ...
        else if (obj.displayName && obj.displayName !== "null")
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!onboardingOverlay) {
    ...
    setTimeout(() => {
      if (typeof checkAndPromptNotifications === "function")
        checkAndPromptNotifications();
    }, 3000);
    return;
  }
  ...
  if (!hasSeenIntro) {
    ...
    carouselElement.addEventListener("slide.bs.carousel", function (e) {
      if (e.to === 5) {
        // الشريحة السادسة والأخيرة (تبدأ من 0)
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const movePopupToFullscreen = (event) => {
    ...
    if (fullscreenElement && !fullscreenElement.contains(popupElement)) {
      ...
      setTimeout(() => {
        ...
        const backdropClass = event.type.includes("modal")
          ? ".modal-backdrop"
          : ".offcanvas-backdrop";
  ...
  const returnPopupToBody = (event) => {
    ...
    if (
      ...
      popupElement.parentElement !== document.body
    ) {
...
window.toggleQuickJump = function () {
  ...
  const namesArray =
    typeof surahNames !== "undefined" ? surahNames : window.surahNames;
...
window.jumpToSurah = function (surahNum) {
  ...
  const pageMapArray =
    typeof surahPageMap !== "undefined" ? surahPageMap : window.surahPageMap;
