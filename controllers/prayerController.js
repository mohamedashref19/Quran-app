...
exports.getPrayerTimes = catchAsync(async (req, res, next) => {
  ...
  if (!lat || !lng) {
    return next(
      new AppError("Please provide latitude (lat) and longitude (lng)!", 400),
    );
  }

  const appSettings = await Setting.findOne();
  const hijriOffsetInDays = appSettings ? appSettings.hijriOffset : 0;
  ...
  if (date) {
    ...
    const p = {};
    offsetStr.forEach(({ type, value }) => {
      p[type] = value;
    });
    dateObj = new Date(
      `${p.year}-${p.month}-${p.day}T${p.hour === "24" ? "00" : p.hour}:${
        p.minute
      }:${p.second}`,
    );
  }
  ...
  const hijriDateObj = new Date(
    dateObj.getTime() + hijriOffsetInDays * 24 * 60 * 60 * 1000,
  );
  ...
  if (method === "MWL") {
    ...
    if (userTimeZone.includes("Riyadh") || userTimeZone.includes("Saudi")) {
      ...
      userTimeZone.includes("Dubai") ||
      userTimeZone.includes("Muscat")
    ) {
      params = CalculationMethod.Dubai();
      ...
      userTimeZone.includes("Karachi") ||
      userTimeZone.includes("Kabul")
    ) {
      params = CalculationMethod.Karachi();
  ...
  if (methodName.includes("Umm Al-Qura")) {
    try {
      ...
      const hijriMonth = parseInt(hijriMonthFormatter.format(hijriDateObj));
  ...
  switch (nextPrayer) {
    case Prayer.Fajr:
      nextPrayerName = "Fajr";
      nextPrayerTimeRaw = prayerTimes.fajr;
      break;
    case Prayer.Sunrise:
      nextPrayerName = "Sunrise";
      nextPrayerTimeRaw = prayerTimes.sunrise;
      break;
    case Prayer.Dhuhr:
      nextPrayerName = "Dhuhr";
      nextPrayerTimeRaw = prayerTimes.dhuhr;
      break;
    case Prayer.Asr:
      nextPrayerName = "Asr";
      nextPrayerTimeRaw = prayerTimes.asr;
      break;
    case Prayer.Maghrib:
      nextPrayerName = "Maghrib";
      nextPrayerTimeRaw = prayerTimes.maghrib;
      break;
    case Prayer.Isha:
      nextPrayerName = "Isha";
      nextPrayerTimeRaw = prayerTimes.isha;
      break;
    case Prayer.None:
  ...
  res.status(200).json({
    ...
    data: {
      ...
      meta: {
        ...
        method: methodName,
        madhab:
          params.madhab === Madhab.Hanafi ? "Hanafi" : "Shafi/Maliki/Hanbali",
